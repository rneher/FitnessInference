...
class fitness_inference(survival_gen_func):
    ...
    def set_tree(self, tree, time_scale=None):
        ...
        for ci,c in enumerate(self.terminals): c.ladder_rank = ci
        ...
        for node in self.T.get_nonterminals():
            self.time_to_present[node]=np.mean([self.depths[tnode] for tnode 
                                                in node.get_terminals()])-self.depths[node]
    ...
    def guess_time_scale(self):
        ...
        random_pairs  = (np.random.randint(len(self.terminals), size=50),
        ...
        for ci1, ci2 in zip(random_pairs[0], random_pairs[1]):
            ...
            pair_times.append(min(p1[si-1].distance(c1),p2[si-1].distance(c2)))
    ...
    def normalize(self, clade):
        ...
        if np.isnan(np.sum(clade.prob[:])):
            print "encountered nan in ancestral fitness inference in clade ", clade.name
    ...
    def calc_mean_and_variance(self,clade):
        ...
        clade.var_fitness = np.sum(self.fitness_grid**2 * clade.prob)-clade.mean_fitness**2
